...
namespace IBaseFramework.Logger.Logging
{
    public abstract class LogBase : ILog
    {
        ...
        {
            if (IsTraceEnabled)
                _write(LogLevel.Trace, Format(format, args));
        }
        ...
        {
            if (IsDebugEnabled)
                _write(LogLevel.Debug, Format(format, args));
        }
        ...
        {
            if (IsInfoEnabled)
                _write(LogLevel.Info, Format(format, args));
        }
        ...
        {
            if (IsWarnEnabled)
                _write(LogLevel.Warn, Format(format, args));
        }
        ...
        {
            if (IsErrorEnabled)
                _write(LogLevel.Error, Format(format, args));
        }
        ...
        {
            if (IsErrorEnabled)
                _write(LogLevel.Error, Format(format, args), exception);
        }
        ...
        {
            if (IsFatalEnabled)
                _write(LogLevel.Fatal, Format(format, args));
        }
        ...
        {
            if (IsFatalEnabled)
                _write(LogLevel.Fatal, Format(format, args), exception);
        }
